...
import { DashboardViewer } from './components/DashboardViewer.tsx';
    ...
    <div className="flex h-screen w-full overflow-hidden bg-gray-100">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="md:hidden flex items-center justify-between p-4 bg-white border-b border-gray-200">
          ...
          <h1 className="text-lg font-bold text-gray-800 truncate px-4">{selectedDashboard?.name || 'Power BI Hub'}</h1>
          <button onClick={() => setIsSettingsOpen(true)} className="p-2 text-gray-600 hover:bg-gray-100 rounded-lg">
          ...
          {selectedDashboard ? (
            ...
            <div className="h-full flex flex-col items-center justify-center p-8 text-center text-gray-500">
              <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 max-w-md">
                ...
                <button onClick={() => setIsSettingsOpen(true)} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg font-medium">Configurar Tableros</button>
              </div>
            </div>
          )}
        </div>
